// AES-128 implementation for visualization
// Forward S-box
export const SBOX = [
  ,
];

const RCON = [0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36];

function xtime(b) {
  return ((b << 1) ^ (b & 0x80 ? 0x1b : 0)) & 0xff;
}

function gmul(a, b) {
  let p = 0;
  for (let i = 0; i < 8; i++) {
    if (b & 1) p ^= a;
    const hi = a & 0x80;
    a = (a << 1) & 0xff;
    if (hi) a ^= 0x1b;
    b >>= 1;
  }
  return p;
}

// Key expansion — returns array of 11 round keys (each is flat 16-byte array)
export function keyExpansion(keyBytes) {
  const w = [];
  for (let i = 0; i < 4; i++) {
    w.push([keyBytes[4*i], keyBytes[4*i+1], keyBytes[4*i+2], keyBytes[4*i+3]]);
  }
  for (let i = 4; i < 44; i++) {
    let temp = [...w[i-1]];
    if (i % 4 === 0) {
      temp = [SBOX[temp[1]] ^ RCON[i/4-1], SBOX[temp[2]], SBOX[temp[3]], SBOX[temp[0]]];
    }
    w.push(w[i-4].map((b, j) => b ^ temp[j]));
  }
  const roundKeys = [];
  for (let r = 0; r <= 10; r++) {
    const rk = [];
    for (let i = 0; i < 4; i++) rk.push(...w[r*4+i]);
    roundKeys.push(rk);
  }
  return roundKeys;
}

// State is column-major 4x4 (state[col][row])
export function bytesToState(bytes) {
  const s = Array.from({length:4}, () => Array(4).fill(0));
  for (let c = 0; c < 4; c++)
    for (let r = 0; r < 4; r++)
      s[r][c] = bytes[c*4+r];
  return s;
}

export function stateToBytes(s) {
  const out = [];
  for (let c = 0; c < 4; c++)
    for (let r = 0; r < 4; r++)
      out.push(s[r][c]);
  return out;
}

export function subBytes(state) {
  return state.map(row => row.map(b => SBOX[b]));
}

export function shiftRows(state) {
  return state.map((row, r) => {
    const shifted = [...row];
    for (let i = 0; i < r; i++) shifted.push(shifted.shift());
    return shifted;
  });
}

export function mixColumns(state) {
  return state[0].map((_, c) => {
    const col = state.map(row => row[c]);
    return [
      gmul(0x02,col[0])^gmul(0x03,col[1])^col[2]^col[3],
      col[0]^gmul(0x02,col[1])^gmul(0x03,col[2])^col[3],
      col[0]^col[1]^gmul(0x02,col[2])^gmul(0x03,col[3]),
      gmul(0x03,col[0])^col[1]^col[2]^gmul(0x02,col[3]),
    ];
  }).reduce((acc, col, c) => {
    col.forEach((v, r) => { acc[r][c] = v; });
    return acc;
  }, Array.from({length:4}, () => Array(4).fill(0)));
}

export function addRoundKey(state, roundKey) {
  return state.map((row, r) => row.map((b, c) => b ^ roundKey[c*4+r]));
}

// Returns all intermediate states for each round operation
export function aesEncryptSteps(plaintext, key) {
  const roundKeys = keyExpansion(key);
  let state = bytesToState(plaintext);
  const steps = [];

  const snap = (op, st, rk) => steps.push({
    op,
    state: st.map(row => [...row]),
    roundKey: rk ? [...rk] : null,
  });

  snap('initial', state, null);
  state = addRoundKey(state, roundKeys[0]);
  snap('addRoundKey', state, roundKeys[0]);

  for (let round = 1; round <= 10; round++) {
    state = subBytes(state);
    snap('subBytes', state, null);

    state = shiftRows(state);
    snap('shiftRows', state, null);

    if (round < 10) {
      state = mixColumns(state);
      snap('mixColumns', state, null);
    }

    state = addRoundKey(state, roundKeys[round]);
    snap('addRoundKey', state, roundKeys[round]);
  }

  return { steps, ciphertext: stateToBytes(state) };
}

export function hexToBytes(hex) {
  const clean = hex.replace(/\s/g, '');
  const bytes = [];
  for (let i = 0; i < 32 && i < clean.length; i += 2) {
    bytes.push(parseInt(clean.slice(i, i+2), 16) || 0);
  }
  while (bytes.length < 16) bytes.push(0);
  return bytes.slice(0, 16);
}

export function stringToBytes(str) {
  const bytes = new TextEncoder().encode(str);
  const result = new Uint8Array(16).fill(0);
  result.set(bytes.slice(0, 16));
  return Array.from(result);
}

export function bytesToHex(bytes) {
  return bytes.map(b => b.toString(16).padStart(2, '0')).join('');
}
